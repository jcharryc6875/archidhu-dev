...
class ServicioPeer extends BaseServicioPeer
{
    ...
    public static function getAnexoServicios($servicio_id)
    {
        ...
        if (!$viewAll) {
            $c->add(ServicioAnexosPeer::ES_ACTUAL, 1);
        }
    ...
    {
        try {
            if (empty($table_name) || empty($name_column) || empty($servicio_id)) {
                return 0;
            }
            //**********************************************************************************************
            $conexion = Propel::getConnection();
    ...
    public static function asignarUserServicio($id_asignar, $regional_id, $servicio_id, $addNew = false)
    {
        ...
        try {
            ...
            else if (in_array($servicio->getTipoServicio()->getTipoEnvio(), array(2, 5))) {
                $servicioestado_id = 10;
            } //email
            else if (!empty($ureceptor_id)) {
                $servicioestado_id = 2;
            } //receptor asignado
            else {
                $servicioestado_id = 4;
            } //pendiente no tiene receptor
            //**********************************************************************************************
            $notas_email = null;
            if ($addNew) {
    ...
    public static function getListDigitDocument($com_radicado = 0, $radicado_origen = 0, $modulo_id = 0)
    {
        ...
        try {
            if (!empty($com_radicado) || !empty($radicado_origen)) {
                $fpath_resolve = new simad_paths_app();
                    ...
                    foreach ($servicio->getServicioAnexoss() as $serv_attachs) {
                        if ($serv_attachs->getEsActual()) {
                            ...
                            if (trim($files_adjuntos[0])) {
                                $fpath_resolve = simad_paths_app::resolveRelativePath($files_adjuntos[0]);
                                if (file_exists($fpath_resolve)) {
                                    $file_size = filesize($fpath_resolve);
                                }
                                //*********************************************************************
                                ...
                                $list_attach[] = array(
                                    ...
                                    'DESCRIPCION_ADJUNTO' => $serv_attachs->getDescripcion(),
                                    'URL_ADJUNTO' => $url_secure,
                                    'DOWNLOAD_ADJUNTO' => $url_download,
                                    'FECHA_ADJUNTO' => $serv_attachs->getFechaCreacion(),
                                    'SIZE_FILE' => $file_size
                                );
                            }
                        }
