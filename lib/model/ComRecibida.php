        ...
        try {
            ...
            if (count($duplicados_conresp) > 0) {
                if (!empty($duplicados_conresp[0]['RADICADO_COMENVIADA'])) {
                    $radicados_comrecibida = array_unique(array_map(function ($rad_comrec) {
                        return $rad_comrec['RADICADO_COMRECIBIDA'];
                    }, $duplicados_conresp));
                    $radicados_comenviada = array_unique(array_map(function ($rad_comenv) {
                        return $rad_comenv['RADICADO_COMENVIADA'];
                    }, $duplicados_conresp));
                } else {
        ...
        try {
            ...
            if ($unidad_documental == null) {
                ...
                if ($response_process['error'] === false) {
                    ...
                    TransferenciaPeer::addAutoTransfAndContenido(
                        $unidad_documental->getPrimaryKey(),
                        $tipodocumental_id,
                        $this->getPrimaryKey(),
                        OrigenTransferenciaCom::ComRecibida,
                        $usuariologuiado
                    );
                    //*******************************************************************************************
    ...
    public function enviarRespuestaExterna()
    {
        try {
            ...
            if ($this->getPrimaryKey()) {
                ...
                $response_data = $simadSoap->loadWsInfoRadicadoEntrada($this->getPrimaryKey(), false);
            } else {
                ...
                if (strtolower($file_vars['extension']) == 'pdf') {
                    ...
                    if (file_exists($file_stamp)) {
                        ...
                        if (!$response_ws['error']) {
                            ...
                            if ($isFileCreate) {
                                unlink($file_stamp);
                                if (copy($unique_file, $filenametarget)) {
                                    unlink($unique_file);
                                }
                            } else {
    ...
    public function setNewStateByDigitCom($tipoprocesocom_id = 2)
    {
        try {
            if ($this->getEstadocomrecibidaId() == 11) {
                ...
                //****************************************************************************************
                if (!empty($tipoprocesocom_id)) {
                    $this->setTipoprocesocomId($tipoprocesocom_id);
                }
                //****************************************************************************************
