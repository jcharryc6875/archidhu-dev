		...
		try {
			...
			if (file_exists($file_source)) {
				...
				if ($search_xguid == $xguid) {
					return array('NOMBRE_ARCHIVO' => basename($urlfile[0]), 'URL' => trim($file_source), 'TIPO_ATTACHMENT' => 'ANEXO', 'SIZE_FILE' => $file_size);
				} else {
					return array('NOMBRE_ARCHIVO' => null, 'URL' => null, 'SIZE_FILE' => $file_size);
				}
			} else {
				return array('NOMBRE_ARCHIVO' => null, 'URL' => null, 'SIZE_FILE' => $file_size);
			}
		} catch (Exception $ex) {
			return array('NOMBRE_ARCHIVO' => null, 'URL' => null, 'SIZE_FILE' => 0);
		}
	}
