...
{
	$jq_path = sfConfig::get('sf_jquery_web_dir', '/theme/neon/assets');
  	/*$jq_path = sfConfig::get('sf_jquery_web_dir', '/theme/neon/assets') .
    '/js/' . sfConfig::get('sf_jquery_core', 'jquery-1.11.0.min.js');*/
}
 ...
 *    'loading'  => "Element.show('indicator'); \jQuery('tag').value = ''",
 ...
 *        'update'  => 'posts',
 *        'url'     => '@reload',
 *        'complete => visual_effect('fade', '#posts', array('speed' => 'slow', 'callback' => 'function(){jQuery("post_content").css("background", "yellow")}' )),
 *  )) ?>
 *
 ...
 *        'update'  => 'posts',
 *        'url'     => '@reload',
 *        'complete => visual_effect('fadeTo', '#posts', array('speed' => 'slow', 'opacity' => '0.33', 'callback' => 'function(){jQuery("post_content").css("background", "yellow")}' )),
 *  )) ?>
 *
...
function jq_input_auto_complete_tag($name, $value, $url, $tag_options = array(), $completion_options = array()) {
  ...
  jq_add_plugins_by_name(array("autocomplete"));
	...
	$autocomplete_script = sprintf('jQuery("#%s").autocomplete("%s",{ %s	});',$name,$url,$jsonOptions);
