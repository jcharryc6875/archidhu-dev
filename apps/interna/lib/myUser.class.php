 ...
 public function guardarAuditoria($modulo,$campos,$valor_anterior,$valor_nuevo,$codigo_principal)
  {
	...
	public function checkPerm($nombre_forma , $usuario_id=""){
		...
		//$c=new Criteria();
		//$c->add(UnidadDocumentalPeer::UNIDADDOCUMENTAL_ID,$llave);
		//$c->doSelectOne();
        ...
        if($nudebug){ $cuenta = UsuarioPrivilegioPeer::doCount($c); }
        
		$permission = UsuarioPrivilegioPeer::doCount($c);
        ...
        if($permission){
            ...
            $c->add(FormaPeer::NOMBRE,$nombre_forma);
            $permission2 = RolPorUsuarioPeer::doCount($c);
        	if($permission2){
				return true;
			}
		}
