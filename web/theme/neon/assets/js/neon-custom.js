		...
		if($.isFunction($.fn.select2))
		{
			$(".select2").each(function(i, el)
			{
                ...
                if($this.hasClass('required')){
                    $(".select2-default").addClass("required");
                    /*
		...
		if($.isFunction($.fn.daterangepicker))
		{
			$(".daterange").each(function(i, el)
			{
				...
				var ranges = {
					...
					'Ultimos 3 días': [moment().subtract('days', 2), moment()],
				...
				$this.daterangepicker(opts, function(start, end)
				{
					...
					if($this.hasClass('daterange-inline'))
					{
						...
						$('#erecfecha_inicio').val(start.format('YYYY-MM-DD'));
						$('#erecfecha_fin').val(end.format('YYYY-MM-DD'));
					}
				});
			});
		}
			...
			$("form.validate").each(function(i, el)
			{
				var $this = $(el),
					opts = {
						...
						highlight: function (element) {
							...
							$(element).parent().removeClass('validate-has-error').addClass('validate-has-error');
						},
						unhighlight: function (element) {
							//$(element).closest('.form-group').removeClass('validate-has-error');
							$(element).parent().removeClass('validate-has-error').removeClass('validate-has-error');
						},
				...
				$fields.each(function(j, el2)
				{
					...
					for(var k in _validate)
					{
						...
						if($.inArray(rule, ['required', 'url', 'email', 'number', 'date', 'creditcard', 'folder-treename']) != -1)
						{
...
function formatState (opt) {
    if(opt.img == 'undefined'){
        ...
        return '<span><img src="' + opt.img + '" width="20px"/>' + opt.text + '</span>';
    }
}
