<?php
	...
	function ConsultaRadicadoPublic($EntSecurity = array(), $EntComInfo = array())
	{
		...
		try {
			...
			//*****************************************************************************************************************
			$anyParamValid = false;
			if(!empty($radicado_entrada)){ $anyParamValid = true; $info_request[] = "Radicado Entrada: ".$radicado_entrada; }
			if(!empty($radicado_salida)){ $anyParamValid = true; $info_request[] = "Radicado Salida: ".$radicado_salida; }
			if(!empty($numero_identificacion)){ $anyParamValid = true; $info_request[] = "Numero Identificacion: ".$numero_identificacion; }
			if(!empty($numero_expediente)){ $anyParamValid = true; $info_request[] = "Numero Expediente: ".$numero_expediente; }
			//*****************************************************************************************************************
	...
	function getRemitentesList($EntSecurity = array(), $nombre = null,$funcionario = null,$nuid = null,$email = null,$limit_select=25)
	{
		...
		try{
			...
			$list_object[] = array('directorioexterno_id' => 0,
			'ciudad' => null,
			'ciudad_codigo' => null,
			'nuid' => null,
			'direccion' => null,
	...
	function AddRadicadoSalidaEnt($EntSecurity = array(), $EntComEnviada = array(), $EntInteresadoOfList = array() ,$EntRemitente = array())
	{
		...
		try{
			...
			//***************************************************************************************************************
			$estaAsignada = 0;$otherAsigned = false;
	...
	function GetMetadataInfoCom($EntSecurity = array(), $ComSearchInfo = array())
	{
		...
		try{
			...
			if(!empty($cons_entrada)){
				...
				if($com_enviada == null){ $isErrorCom = true; return; }
	...
	function ActualizarExpedienteArch($EntSecurity = array(), $ArcAddExpediente = array())
	{
		...
		try
		{
			...
			//**************************************************************************************************************************
			AuditLogPeer::guardarAuditoriaLite("UnidadDocumental",$unidad_documental_last,$unidad_documental,
				ModulesEnable::Archivo,trim($unidad_documental->getCodigoBarras()),$usuario_ws['object']->getPrimaryKey());
			//**************************************************************************************************************************
