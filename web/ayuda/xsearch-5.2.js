...
function searchAll(keyword){
	...
	document.open()
	document.clear()
	document.write('<link href="CSS/ayudaSIMAD.css" rel="stylesheet" type="text/css" />')
	AddBody()
	...
	document.write("<left>Resultados <b>"+(display_start+1)+"-"+(displast)+"</b> de <b>"+sites+"</b> para <b>"+keyword+"</b> Tiempo de busqueda <b>"+((timeB-timeA)/1000)+"</b> segundos.</h5></center>")

	if (displast>sites && finds!=0) displast=sites+1


	if (finds==0) {
		document.write("<h4><font color=#990000>No tenemos resultados para la palabra <b>''"+keyword+"''</b></font></h4>"+
						"<h5>La palabra - <b>"+keyword+"</b> - no se encuentra en la base de datos del Centro de ayuda Simad</h5>"+
						"<li>Verifique que haya escrito bien la palabra.</li>"+
	...
	for (q=display_start; q<q3; q+=1) {
		if (results[q].val>0) {
			rc=results[q].rec
			document.write("<span>"+records[rc].link+"</span><br>")
			...
			if (x1>0 && x2>0) {
				...
				document.write("<table><tr><td><span>"+records[rc].description+"</span></td></tr></table><span>"+tmp+"</span><br><br>")
			}
			q2++
   		}
	}
...
function AddBody() {
	...
	templateBody()
	document.write("<form name='searchform' method='post' action='javascript:doSearch()'><table><tr><td align='center'><font face=''Source Code Pro',' size='3'><input name='searchwords' type='text' size='30' ><br><a href='javascript:doSearch()'>"+ButtonCode+"</a></font></td></tr></table></form><hr size=1>")
	if (usebannercode) bannerCode()
}

function DisplayXSearch() {
	// Copyright by <Pixel GD> www.pixel-gd.net.
