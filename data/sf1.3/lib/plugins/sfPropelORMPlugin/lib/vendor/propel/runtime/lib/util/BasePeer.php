...
class BasePeer
{
    ...
    public static function doUpdate(Criteria $selectCriteria, Criteria $updateValues, PropelPDO $con)
     {
        ...
        foreach ($updateTablesColumns as $tableName => $columns) {
            ...
            try {
                ...
                if (!empty($columns)) {
                    ...
                    foreach ($selectCriteria->getJoins() as $join) {
                        $join->setDB($db);
                        $joinTables[] = $join->getRightTableWithAlias();
                        $joinClause[] = $join->getClause($params);
                    }
                    $sql .= count($joinTables) ? " FROM ".$tableName." ".implode(" ", $joinClause) : "";

                    foreach ($tablesColumns as $stable) {
                        foreach ($stable as $tablewh => $colName) {
                            $sb = "";
    ...
    public static function doUpdateOriginal(Criteria $selectCriteria, Criteria $updateValues, PropelPDO $con)
    {
        $db = Propel::getDB($selectCriteria->getDbName());
        $dbMap = Propel::getDatabaseMap($selectCriteria->getDbName());
    ...
    public static function createSelectSql(Criteria $criteria, &$params)
    {
        ...
        $propel = $config['datasources']['propel'];
        $withnolock = ' ';
		
        if(isset($propel['adapter'])){
            if($propel['adapter'] == "sqlsrv"){ $withnolock = ' WITH (NOLOCK) '; }
        }
		
		$from .= $joinClause ? $withnolock . implode(' ', $joinClause) : $withnolock;
